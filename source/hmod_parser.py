...
from hmod_validator import validate_hmod
...
def build_xml_string(sbml):
    ...
    xmlstr = minidom.parseString(ET.tostring(sbml, xml_declaration=True, encoding="UTF-8")). \
        toprettyxml(indent="\t", encoding="UTF-8")

    # necessary adjustments to make the produced file cleaner
    string_to_write = xmlstr.decode("utf-8"). \
    ...
    string_to_write = "".join([s for s in string_to_write.strip().splitlines(True) if s.strip()])
...
def get_local_parameters_dict(lines):
    ...
    par = [get_value_from_string(i) for i in lines if "parameters(" in i and "id" in i and i not in global_parameters]
    par_values = [get_value_from_string(i) for i in lines if
                  "parameters(" in i and "val" in i and i not in global_parameters]
...
def get_attribute_list(lines, category, category_number):
    ...
    x = [attributes_list[i:i + category_number] for i in range(0, len(attributes_list), category_number)]
    return x
...
def store_species(species, list_of_species):
    ...
    for i in species:
        specie = ET.SubElement(list_of_species, "species",
                               metaid="metaid_" + str(meta_id),
                               id=i[0],
                               compartment=i[2],
...
def store_parameters(lines, list_of_parameters):
    ...
    parameters = get_global_parameters(lines)
    ...
    x = [parameters[i:i + parameters_number] for i in range(0, len(parameters), parameters_number)]
...
def store_rate_rules(rate_rules, list_of_rate_rules):
    ...
    for i in rate_rules:
        compartment = ET.SubElement(list_of_rate_rules, "rateRule",
                                    metaid="metaid_" + str(meta_id),
                                    variable=i[0])

        # builds the MathML
        equation = i[1]
        eq = fromstring(translateInfix(equation).replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", ""))
...
def store_assignment_rules(assignment_rules, list_of_assignment_rules):
    ...
    for i in assignment_rules:
        assignment_rule = ET.SubElement(list_of_assignment_rules, "assignmentRule",
                                        metaid="metaid_" + str(meta_id),
                                        variable=i[0])
...
def store_reactions(lines, list_of_reactions, species_list):
    ...
    x = [reactions[i:i + reactions_number] for i in range(0, len(reactions), reactions_number)]

    # processing each reaction
    for i in x:

        reaction = ET.SubElement(list_of_reactions, "reaction",
                                 metaid="metaid_" + str(meta_id),
                                 id=get_value_from_string(i[0]))
        meta_id += 1
        ...
        if len(reactants) > 0:
            ...
            for key, value in reactants.items():
                speciesReference = ET.SubElement(listOfReactants, "speciesReference",
                                                 metaid="metaid_" + str(meta_id),
        ...
        if len(products) > 0:
            ...
            for key, value in products.items():
                speciesReference = ET.SubElement(listOfProducts, "speciesReference",
                                                 metaid="metaid_" + str(meta_id),
                                                 species=species_list[key],
                                                 stoichiometry=str(value))
                meta_id += 1
        ...
        modifiers = [i.text.strip() for i in
                     eq.findall(".//{http://www.w3.org/1998/Math/MathML}ci") if i.text.strip() in species_list]
        ...
        for modifier in modifiers:
            modifierSpeciesReference = ET.SubElement(listOfModifiers, "modifierSpeciesReference",
                                                     metaid="metaid_" + str(meta_id),
                                                     species=modifier)
            meta_id += 1
        ...
        parameters = [i.text.strip() for i in
                      eq.findall(".//{http://www.w3.org/1998/Math/MathML}ci") if i.text.strip() in local_parameters]
        ...
        if len(parameters) > 0:
            ...
            for param in parameters:
                parameter = ET.SubElement(listOfParameters, "parameter",
                                          metaid="metaid_" + str(meta_id),
                                          id=param,
...
def store_events(events, list_of_events):
    ...
    for i in events:
        ...
        eq = fromstring(translateInfix(trigger_equation).replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", ""))
        trigger.append(eq)

        # builds the <delay> element
        delay = ET.SubElement(event, "delay")
        delay_equation = get_value_from_string(i[2])
        eq = fromstring(translateInfix(delay_equation).replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", ""))
        delay.append(eq)
...
def store_event_assignments(event, event_assignments):
    ...
    x = [event_assignments[i:i + event_assignments_number] for i in
         range(0, len(event_assignments), event_assignments_number)]

    # processing each event assignment
    for i in x:
        event_assignment = ET.SubElement(listOfEventAssignments, "eventAssignment",
                                         metaid="metaid_" + str(meta_id),
        ...
        eq = fromstring(
            translateInfix(event_assignment_equation).replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", ""))
        event_assignment.append(eq)


def main(filename):
    # open hmode file
    hmod_file = open(filename, "r")

    # get every line of the file into a list
    lines = hmod_file.readlines()
    ...
    # build the <sbml>
    sbml = ET.Element("sbml", xmlns="http://www.sbml.org/sbml/level2/version4", level="2", version="4")

    # build the <model>
    model = ET.SubElement(sbml, "model", id=filename.split("/")[-1].replace(".hmod", ""))
    ...
    store_parameters(lines, listOfParameters)
