...
from xml_validator import validate_sbml
...
def write_species(root, namespace, filename):
    ...
    for i, specie in enumerate(species):
        ...
        if "id" in specie.attrib:
            species_string += ('{}.species({}).id="{}";\n'.format(filename, count, specie.attrib["id"]))

        # specie val
        if "initialConcentration" in specie.attrib:
            species_string += (
                '{}.species({}).val={};\n'.format(filename, count, specie.attrib["initialConcentration"]))

        # specie compartment
        if "compartment" in specie.attrib:
            species_string += (
                '{}.species({}).compartment="{}";\n'.format(filename, count, specie.attrib["compartment"]))

        # specie fixed
        if "constant" in specie.attrib:
            species_string += (
                '{}.species({}).fixed={};\n'.format(filename, count, 0 if specie.attrib["constant"] == "false" else 1))
        else:
            species_string += (
...
def write_parameters(root, namespace, filename):
    ...
    local_parameters = root.findall(
        './*/{{{}}}listOfReactions/*/{{{}}}kineticLaw/{{{}}}listOfParameters/*'.format(namespace, namespace, namespace))
    local_parameters += root.findall(
        './*/{{{}}}listOfReactions/*/{{{}}}kineticLaw/{{{}}}listOfLocalParameters/*'.format(namespace, namespace,
                                                                                            namespace))
    parameters_string += ("{}.nparameters={};\n".format(filename, len(parameters + local_parameters)))
    param_counter = 0
    ...
    for i, parameter in enumerate(parameters):
        ...
        if "id" in parameter.attrib:
            parameters_string += (
                '{}.parameters({}).id="{}";\n'.format(filename, param_counter, parameter.attrib["id"]))

        # parameter val
        if "value" in parameter.attrib:
            parameters_string += (
                '{}.parameters({}).val={};\n'.format(filename, param_counter, parameter.attrib["value"]))

        # parameter fixed
        if "constant" in parameter.attrib:
            parameters_string += (
                '{}.parameters({}).fixed={};\n'.format(filename, param_counter, 0 if parameter.attrib["constant"] == "false" else 1))
        else:
            parameters_string += (
                '{}.parameters({}).fixed={};\n'.format(filename, param_counter, 0))

        # parameter reaction
        parameters_string += ('{}.parameters({}).reaction="{}";\n'.format(filename, param_counter, "global"))
    ...
    for local_parameter in local_parameters:
        ...
        if "id" in local_parameter.attrib:
            parameters_string += (
                '{}.parameters({}).id="{}";\n'.format(filename, param_counter, local_parameter.attrib["id"]))

        # local parameter val
        if "value" in local_parameter.attrib:
            parameters_string += (
                '{}.parameters({}).val={};\n'.format(filename, param_counter, local_parameter.attrib["value"]))

        # parameter fixed
        if "constant" in local_parameter.attrib:
            parameters_string += (
                '{}.parameters({}).fixed={};\n'.format(filename, param_counter,
        ...
        parameters_string += ('{}.parameters({}).reaction="{}";\n'.format(filename, param_counter, "local"))
...
def write_assignment_rules(root, namespace, filename):
    ...
    for i, assignment_rule in enumerate(ruleAss):
        ...
        if "variable" in assignment_rule.attrib:
            assignment_rules_string += (
                '{}.ruleAss({}).id="{}";\n'.format(filename, count, assignment_rule.attrib["variable"]))

        # assignment_rule val
        # val = translate_mathml(assignment_rule.find(
        #     './{http://www.w3.org/1998/Math/MathML}math/*'))
        rule = tostring(assignment_rule.find(
            './{http://www.w3.org/1998/Math/MathML}math'), encoding="unicode", method="xml")
    ...
    for i, event in enumerate(events):
        ...
        for j, event_assignement in enumerate(event_assignements):
            ...
            if "variable" in event_assignement.attrib:
                events_string += (
                    '{}.events({}).var({})="{}";\n'.format(filename, count, ass_count,
                                                           event_assignement.attrib["variable"]))

            # event assignement var
            # var = translate_mathml(event_assignement.find(
            #     './{http://www.w3.org/1998/Math/MathML}math/*'))
            math_var = tostring(event_assignement.find('./{http://www.w3.org/1998/Math/MathML}math'),
                                encoding="unicode", method="xml")
            var = translate_mathml(math_var)
            events_string += ('{}.events({}).var({})="{}";\n'.format(filename, count, ass_count, var))
...
def write_reactions(root, namespace, filename):
    ...
    for i, reaction in enumerate(reactions):
        ...
        if "id" in reaction.attrib:
            reactions_string += ('{}.reaction({}).id="{}";\n'.format(filename, count, reaction.attrib["id"]))
        ...
        math_rate = tostring(reaction.find('./{{{}}}kineticLaw/{{http://www.w3.org/1998/Math/MathML}}math'.format(namespace)),
                             encoding="unicode", method="xml")
        ...
        reactants_dict = {k.attrib["species"]: k.attrib["stoichiometry"] if "stoichiometry" in k.attrib else "1" for k
                          in reactants_list}

        products_list = reaction.findall('./{{{}}}listOfProducts/*'.format(namespace))
        products_dict = {k.attrib["species"]: k.attrib["stoichiometry"] if "stoichiometry" in k.attrib else "1" for k in
                         products_list}
        ...
        # reaction values
        reactions_string += (
            '{}.reaction({}).{}="{}";\n'.format(filename, count, reaction.attrib["id"], list(values.values())))
...
def write_rate_rules(root, namespace, filename):
    ...
    for i, rate_rule in enumerate(rateRules):
        ...
        if "variable" in rate_rule.attrib:
            rate_rules_string += ('{}.raterules({}).id="{}";\n'.format(filename, count, rate_rule.attrib["variable"]))

        # assignment_rule val
        # val = translate_mathml(rate_rule.find(
        #     './{http://www.w3.org/1998/Math/MathML}math/*'))

        rule = tostring(rate_rule.find(
            './{http://www.w3.org/1998/Math/MathML}math'), encoding="unicode", method="xml")
        val = translate_mathml(rule)
...
def main(filename):
    a = validate_sbml(filename)
    if a != 0:
        return "The SBML file is not valid. See the reason(s) below: \n\n\n" + a
    ...
    f += write_species(root, namespace, hmod_filename)
    f += write_compartments(root, namespace, hmod_filename)
    f += write_parameters(root, namespace, hmod_filename)
