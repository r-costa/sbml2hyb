...
def main(
    ...
    weights = (
        model.get_weights()
    )  # list of numpy arrays containing weights in a per layer/bias basis
    ...
    for layer in range(nlayers):
        ...
        nH = len(
            weights[layer * 2 + 1]
        )  # number of nodes in the layer. Read from the number of bias values, since it is currently assumed to be one per node (Standard dense layers)
        if layer == 0:
            for node in range(nH):
                ...
                for n in range(nin):
                    ...
                    if n == nin - 1:
                        H[layer][node] = (
                            H[layer][node] + "+w" + str(1 + node + nH * (n + 1))
                        )
                        H[layer][node] = "tanh(" + H[layer][node] + ")"

            wcount = (
                wcount + len(H[layer]) * len(inputs) + len(H[layer])
            )  # add number of parsed on this step nodes to count
                ...
                for n in range(len(H[layer - 1])):
                    ...
                    if n == len(H[layer - 1]) - 1:
                        H[layer][node] = (
                            H[layer][node]
                            + "+w"
                            + str(wcount + 1 + node + nH * (n + 1))
                        )
                        H[layer][node] = "tanh(" + H[layer][node] + ")"

            wcount = (
                wcount + len(H[layer]) * len(H[layer - 1]) + len(H[layer])
            )  # add number of parsed on this step nodes to count
    ...
    o_name = (
        curr_file_name.split(".hmod")[0]
        if ".hmod" in curr_file_name
        else curr_file_name.split(".xml")[0]
    )
    n_name = o_name
    ...
    for line in f:
        if ".nparameters=" in line:
            ...
            nhybpar = int(
                line[pos + 1 : -1]
            )  # Define a counter for the hybrid parameters
            ...
            nhybAss = int(
                line[pos + 1 : -1]
            )  # Define a counter for the hybrid assignments

        elif (".ruleAss(" in line) and (
            ").id" in line
        ):  # check for assignment rules to substitute
    ...
    for line in f:
        if (
            ".parameters(" + str(nbasepar) + ").reaction" in line
        ):  # Check if the last standard parameter line has been reached
            if outhyb == 1:
                outhyb = 0
                h += line.replace(o_name, n_name).replace("local", "global") + "\n"
            else:
                h += (
                    line.replace(o_name, n_name) + "\n"
                )  # write the last standard line for the hybrid model
            for w in orderedweights:  # Add all hybrid weights
                nhybpar = (
                    nhybpar + 1
                )  # Increase hybrid parameter counter. Following 4 lines write the new hybrid parameter
                h += (
                    n_name
                    + ".parameters("
                    + str(nhybpar)
        ...
        elif ".ruleAss(" + str(nbaseAss) + ").val" in line or (
            "nruleAss=0" in line
        ):  # Check if the last standard assignment line has been reached
            if skip > 1:
                skip = skip - 1
            elif skip == 0 and ("nruleAss=0" in line):
                h += (
                    line.replace(
                        str(nbaseAss), str(nbaseAss + len(hybout) - subAss)
            ...
            for par in hybout:  # Add all hybrid assignments
                ...
                nhybAss = (
                    nhybAss + 1
                )  # Increase hybrid assignments counter. Following 4 lines write the new hybrid assignments
                h += (
                    n_name
                    + ".ruleAss("
                    + str(nhybAss)
            ...
            h += (
                line.replace(
                    str(nbasepar), str(nbasepar + len(orderedweights))
                ).replace(o_name, n_name)
                + "\n"
            )
