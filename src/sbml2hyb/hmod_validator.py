...
def match_reaction_pattern(line, expected_value):
    reaction_string = rf"^[a-zA-Z0-9\_]*\.reaction\([0-9]*\)\.{expected_value}=(\"|')\[[a-zA-Z0-9 \/\*\+\-\'\,\"\.]*\](\"|');$"
...
def add_to_hmod(category, lines, hmod):
    if category == "species":
        hmod.add_specie(
            get_value_from_parameter(".id=", lines),
        ...
        hmod.add_reaction(
            ...
            get_value_from_parameter(
                f".{get_value_from_parameter('.id=', lines)}=", lines
            ),
        )
...
def validate_category(category, lines, hmod):
    ...
    try:
        ...
        return (
            f"Error in n{category}. Please confirm that this line is correctly written."
        )

    if (
        len(category_lines) == 1
        and category != "events"
        and category != "raterules"
        and category != "ruleAss"
    ):
        # O nspecies tem de ser 0. so deve ser possivel para eventos e rate rules
        return f"Error in {category}. Cannot have 0 values."
    if category_number == 0 and (
        category == "events" or category == "raterules" and category == "ruleAss"
    ):
        return 0
    ...
    if len(divided_species) != category_number:
        return (
            f"Error in n{category} value. It does not match with the rest of the {category} values.\n"
    ...
    for count, i in enumerate(divided_species):
        ...
        correct_parameters = check_parameters(
            parameters, get_parameters_from_list(current_specie), category, count
        )
